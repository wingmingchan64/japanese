<?php
/*
php H:\japanese\programs\wadoku\生成和獨形容詞.php
*/
...
foreach( $id_xml_entry as $id => $xml )
{
	...
	if( $gramGrp_dom[ 0 ] && $gramGrp_dom[ 0 ]->firstElementChild )
	{
		...
		if( $品詞名 == 'keiyoushi' )
		{
			...
			if( !in_array( $kanji, $形容詞陣列 ) )
			{
				array_push( $形容詞陣列, $kanji );
			}
		}
	}
}

foreach( $形容詞陣列 as $形容詞 )
{
	$contents .= "\"$形容詞\",\r\n";
}
...
file_put_contents( "H:\\japanese\\programs\\wadoku\\形容詞.php", $contents );
?>
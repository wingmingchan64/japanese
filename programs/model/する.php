<?php
$する=array(
'Present'=>"${stem}する,${stem}します,${stem}しない,${stem}しません",
'Past'=>"${stem}した,${stem}しました,${stem}しなかった,${stem}しませんでした",
'-te Form'=>"${stem}して,${stem}しなくて",
'Volitional'=>"${stem}しよう,${stem}しましょう",
'Potential'=>"${stem}できる,${stem}できます,${stem}できない,${stem}できません",
'Passive'=>"${stem}される,${stem}されます,${stem}されない,${stem}されません",
'Causative'=>"${stem}させる,${stem}させます,${stem}させない,${stem}させません",
'Imperative'=>"${stem}せよ/${stem}しろ,${stem}してください,${stem}するな,${stem}しないでください",
'Condition'=>"${stem}すれば,${stem}しなければ",
'Condition (-tara)'=>"${stem}したら,${stem}しなかったら",



);
?>